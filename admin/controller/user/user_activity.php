...
class ControllerUserUserActivity extends Controller {
	public function index() {
		...
		foreach ($results as $result) {
		    $user_data = json_decode($result['data'], true);

			$comment = sprintf($this->language->get('text_' . $result['key']), $user_data['name'],$result['key'], $user_data['setting_dataKey']);
	...
	public function addActivityEditCategory(&$route, &$data) {
	...
	public function addActivityEditProduct(&$route, &$data) {
	...
	public function addActivityEditSetting(&$code, &$data, &$store_id) {
		...
		$activity_data = array(
                    ...
                    'setting_dataKey' => $data[0],
                    'setting_data' => $data,
                    'store_id' => is_null($store_id) ? 0 : $store_id
				);
